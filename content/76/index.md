+++
title = '🔎 #76 - Contrary Cotton Swab 👂'
date = 2026-03-14
draft = false
expiryDate = 2026-06-12
tags = [
'HARD',
]
+++
Also known as a Q-tip...
<!--more-->
![Static Badge](https://img.shields.io/badge/hard-red?style=for-the-badge)
![Find](../images/76-find.jpg)
Best not to think about whether it's been used or not...
### ❓Hints
{{% details title="👀" closed="true" %}}
- you can only see one half of it
- ➡️
{{% /details %}}
### 🎯 Location
{{% details title="👀" closed="true" %}}
{{% details title="⚠️ Spoilers ahead! Tap/click to continue to the object location ⚠️" closed="true" %}}
![Found](../images/76-found.jpg)
{{% /details %}}
{{% /details %}}