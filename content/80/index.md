+++
title = '🔎 #80 - Fishy Fishing Lead 🎣'
date = 2026-03-18
draft = false
expiryDate = 2026-06-16
tags = [
'HARD',
]
+++
<!--more-->
![Static Badge](https://img.shields.io/badge/hard-red?style=for-the-badge)
![Find](../images/80-find.jpg)
This is a fishing line lead. It's a thicker line that attaches the hook to the rest of the line
### ❓Hints
{{% details title="👀" closed="true" %}}
- it looks like a dog/cat whisker
- it's black and white
{{% /details %}}
### 🎯 Location
{{% details title="👀" closed="true" %}}
{{% details title="⚠️ Spoilers ahead! Tap/click to continue to the object location ⚠️" closed="true" %}}
![Found](../images/80-found.jpg)
{{% /details %}}
{{% /details %}}